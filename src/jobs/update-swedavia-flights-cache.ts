import {
  Flight,
  SqliteSwedaviaFlightsCache,
} from "../models/sqlite-swedavia-flights-cache.js";
  ...
  for (const airport of airports) {
    for (const direction of directions) {
      for (const date of dates) {
        try {
          logger.info(
            { airport, direction, date },
            `Updating cache for ${airport} ${direction} on ${date}`,
          );
          const rawFlightData = await SwedaviaFlightsClient.get(
            airport,
            direction,
            date,
          );
          const flights = mapSwedaviaResponseToFlights(rawFlightData);
          swedaviaFlightsCache.upsert(
            airport,
            date,
            direction,
            flights,
          ...
          logger.info(
            { airport, direction, date, n: rawFlightData.flights.length },
            `Cache updated for ${airport} ${direction} on ${date} (${rawFlightData.flights.length} flights)`,
          );
        } catch (err) {
          logger.error(
            { airport, direction, date, err },
            `Failed to update cache for ${airport} ${direction} on ${date}`,
          );
        }
      }
    }
